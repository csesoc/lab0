class SQLQuery:
    class solves:
        createTable = """CREATE TABLE IF NOT EXISTS ctf_solves (
    ...
    class questions:
        createTable = """CREATE TABLE IF NOT EXISTS ctf_questions (
    ...
    UNIQUE (title)
        ...
        edit = """UPDATE ctf_questions
            SET title = ?, description = ?, value = ?, category = ?
            WHERE id = ?""",
        editFlag = """UPDATE ctf_qustions
            SET flag = ?
            WHERE id = ?
            """,
