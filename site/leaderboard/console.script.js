...
new Typed("#title", {
	strings: ["><span id='blinky'>_</span> CTF Leaderboard<br>========================="],
	cursorChar: '',
	onComplete: function () {
		leaderboardProvider(undefined, async function (data, ready) {
			...
			typedText = data.map(entry => `<span>#${entry.rank}</span> <span>${entry.name}</span> - <span>${entry.points}</span>`);
			...
			let clearLoop = setInterval(function () {
				...
				if (typed.el.innerText.length == 0) {
					...
					typed.reset();
					typed.options.onComplete = () => { setTimeout(ready, 2 * 1000) };
				}
			}, 1);
		});
	}
